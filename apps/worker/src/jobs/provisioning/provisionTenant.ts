...
import { createTenantDb, users, gradingBands, MATRIC_LAHORE_BOARD_GRADING_BANDS } from "@school-os/db-tenant";
...
export async function provisionTenant(job: ProvisionTenantJob) {
  ...
  try {
    ...
    // 3. Seed default data: grading scheme (Milestone 6, Phase 1 decision
    // #3) and the owner account. Role/permission templates live in code
    // (packages/permissions), not seeded data — see that package's own note.
    await tenantDb.insert(gradingBands).values(MATRIC_LAHORE_BOARD_GRADING_BANDS);
